...
#theorem[
  ...
  $
    D Phi_vb(x)(vb(h)) &= sum_(i=1)^d {
      integral_(t_0)^(t_1) (
        pdv(L, x_i)(vb(x(t)), vb(dot(x))(t), t) - dv(, t)pdv(L, v_i)(vb(x)(t), vb(dot(x))(t),t)
      ) h_i (t) dd(t) \ &+ [pdv(L, v_i)(vb(x)(t), vb(dot(x))(t), t) h_i (t)]_(t_0)^(t_1)
    }
  $
...
$
  D Phi_vb(x) (vb(h)) = sum_(i=1)^d {
    integral_(t_0)^(t_1) (pdv(L, x_i)-dv(, t)pdv(L, v_i)) h_i dd(t) + [pdv(L, v_i) h_i]_(t_0)^(t_1)
  }
$
oppure
$
  D Phi_vb(x) (vb(h)) = integral_(t_0)^(t_1) (grad_vb(x) L - dv(, t) grad_vb(v) L) dprod vb(h) dd(t) + [
    grad_vb(v) L dprod vb(h)
  ]_(t_0)^(t_1).
$
...
#dim[
  ...
  $
    L(vb(x) + difference(vb(x)), vb(dot(x))+ difference(vb(v)), t+ difference(t)) - L(
      vb(x), vb(dot(x)), t
    ) &= sum_(i=1)^d {
      pdv(L, x_i) (vb(x), vb(dot(x)), t) difference(x_i) + pdv(L, v_i) (vb(x), vb(dot(x)), t) difference(v_i)
    } \ &+ pdv(L, t) (vb(x), vb(v), t) difference(t) + o(abs(difference(x_i, v_i, t)))
  $

  $
    ==> Phi(vb(x)+ vb(h)) - Phi(vb(x)) = integral_(t_0)^(t_1) L(vb(x) + vb(h), vb(dot(x)) + vb(dot(h)), t) - L(
      vb(x), vb(dot(x)), t
    ) dd(t)
  $
  $
    = integral_(t_0)^(t_1) sum_(i=1)^d {
      pdv(L, x_i) (vb(x), vb(dot(x)), t) h_i (t) + pdv(L, v_i) (vb(x), vb(dot(x)), t) dot(h)_i (t)+ o(
        abs(h_i (t)), abs(dot(h)_i (t))
      )
    } dd(t), forall i in {1,...,d}.
  $

  Si osserva che, tramite integrazione per parti,
  $
    integral_(t_0)^(t_1) pdv(L, v_i) dot(h)_i dd(t) = [
      pdv(L, v_i) h_i
    ]_(t_0)^(t_1) - integral_(t_0)^(t_1) dv(, t) pdv(L, v_i) h_i dd(t).
  $

  Si considera $R(h) = o(norm(vb(h))_(C^1))$, che implica che $R(h)(t) <= epsilon norm(vb(h))_(C^1)$
  $
    ==> abs(integral_(t_0)^(t_1) R(vb(h))(t) dd(t)) <= integral_(t_0)^(t_1) R(vb(h))(
      t
    )dd(t) <= integral_(t_0)^(t_1) epsilon norm(vb(h))_(C^1) dd(t) = epsilon (t_1-t_0) norm(vb(h))_(C^1).
  $

  Infine, sostituendo,
  $
    Phi(vb(x)+ vb(h)) - Phi(vb(x)) = sum_(i=1)^d {
      integral_(t_0)^(t_1) (pdv(L, x_i)-dv(, t)pdv(L, v_i)) h_i dd(t) + [pdv(L, v_i) h_i]_(t_0)^(t_1)
    } + o(norm(vb(h))_(C^1))
  $
...
#theorem("equazioni di Eulero-Lagrange")[
...
#dim[
  ...
  $
    exists delta, c > 0 : (overline(t)-delta, overline(t)+delta) subset [t_0,t_1] : forall t in (
      overline(t)-delta, overline(t)+delta
    ), f_overline(dotless.i) (t) >= c.
  $
  ...
  $
    integral_(t_0)^(t_1) h_overline(dotless.i) (
      t
    ) dd(t) = integral_(overline(t) - delta)^(overline(t) + delta) h_overline(dotless.i) (t) dd(t).
  $
  Si prende poi $h_i = 0, forall i != overline(dotless.i)$. Infine,
  $
    0 = integral_(t_0)^(t_1) sum_(i=1)^d f_i (t) h_i (
      t
    ) dd(t) = integral_(overline(t) - delta)^(overline(t) + delta) f_overline(dotless.i) (t) h_overline(dotless.i) (
      t
    ) dd(t) >= c integral_(overline(t) - delta)^(overline(t) + delta) h_overline(dotless.i) (t) > 0.
  $
]

#dim("del teorema precedente")[ // Da rivedere
  ...
  $
    Phi(vb(x)+ vb(h)) - Phi(vb(x)) = sum_(i=1)^d integral_(t_0)^(t_1) (pdv(L, x_i)-dv(, t)pdv(L, v_i)) h_i dd(t) + o(
      vb(h)
    ).
  $
...
#theorem("principio di minima azione di Hamilton")[
...
#example[
  ...
  $
    ==> 0 = dv(, t) vb(dot(r)) / norm(vb(dot(r))) = (norm(vb(dot(r)))^2 vb(diaer(r)) - (
      vb(dot(r)) dprod vb(diaer(r))
    )vb(dot(r))) / norm(vb(dot(r)))^3
  $
...
con
$
  dot(q)_i (t) = dv(, t) q_i (r_1 (t), ..., r_d (t)) = sum_(j=1)^d pdv(q_i, r_j) (vb(r)(t)) dot(r)_j (t) = sum_(j=1)^d [
    pdv(vb(q), vb(r)) (vb(r)(t))
  ]_(i j) dot(r)_j (t)
$
...
#def[
  ...
  $ cal(L)(vb(q), vb(dot(q)), t) = L(vb(r(q)), [pdv(vb(q), vb(r))(vb(r))] vb(dot(r)), t). $
]
...
#corollary[
  ...
  rispetto al quale la funzione lagrangiana sia $cal(L)$ come sopra definito. In
  queste coordinate le leggi del moto soddisfano
  $ dv(, t) pdv(cal(L), dot(q)_i) - pdv(cal(L), q_i) = 0 $
]

#dim[
  Si nota che, per costruzione,
  $
    integral_(t_0)^(t_1) L(vb(r)(t), vb(dot(r))(t), t) dd(t) = integral_(t_0)^(t_1) cal(L)(
      vb(q)(t), vb(dot(q))(t), t
    ) dd(t)
  $
  si conclude con il fatto che, per il principio di minima azione di Hamilton, se
  l'integrale è un estremale, allora valgono le equazioni di Eulero-Lagrange per $cal(L)$.
...
#example("applicazione del corollario")[
  ...
  $ cal(T)(r, phi, dot(r), dot(phi)) = m / 2 (dot(r)^2 + r^2 dot(phi)^2) = cal(T)(r, dot(r), dot(phi)) $
  $ ==> cal(L)(r, phi, dot(r), dot(phi)) = m / 2 (dot(r)^2 + r^2 dot(phi)^2) - u(r) = cal(L)(r, dot(r), dot(phi)) $
  ...
  $ dv(, t) pdv(cal(L), dot(r)) - pdv(cal(L), r) = 0 $
  ...
  angolare:
  $ dv(, t) pdv(cal(L), dot(phi)) - pdv(cal(L), phi) = 0 $
...
- un oggetto del tipo $pdv(cal(L), q_i)$ è detto _impulso_ (o _momento_) _generalizzato_;
- una variabile $q_i$ da cui $L$ non dipende è detta _variabile ciclica_. In
  questo caso, l'impulso generalizzato $pdv(cal(L), dot(q)_i)$ si conserva,
  perché $dv(, t) pdv(cal(L), dot(q)_i) = pdv(cal(L), q_i) = 0$.
...
#example[
  ...
  con
  $
    dv(, t) pdv(U, v_x) = -q dv(A_x, t) = -q (
      pdv(A_x, x) dot(x) + pdv(A_x, y) dot(y) + pdv(A_z, z) dot(z) + pdv(A_x, t)
    )
  $
  e
  ...
  $
    ==> dv(, t) pdv(U, v_x) - pdv(U, x) = q(
      v_y (pdv(A_y, x)-pdv(A_x, y)) + v_z (pdv(A_z, x)-pdv(A_x, z)) - pdv(Phi, x) - pdv(A_x, t)
    ) =
  $
